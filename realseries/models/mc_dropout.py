...
class MC_dropout(BaseModel):
    ...
    Attributes:
        model: regular MC-dropout model.
    """
    def __init__(
            self,
            kernel_type='LSTM',
        ...
        self.rnn_model = rnn_base(kernel_type,input_size,\
            hidden_sizes,prediction_window_size,activation,dropout_rate,False).to(self.device)
        self.model = None
    ...
    def fit(self,
        ...
        Args:
            ...
            verbose (bool, optional): Verbose decides what to print. Defaults to
                True.
        Attributes:
            model: a trained model to save to model_path/checkpoint_mc.pt.
        """
        ...
        self.save_model(Path(self.model_path, 'checkpoint_mc.pt'))

    
    def forecast(self, scaler, test_data, t=1, confidence=95, mc_times=400, is_uncertainty=True):
        ...
        self.model.eval()
        pre_arry = self.model(test_data)
        pre_arry = pre_arry.cpu().data.numpy()
        pre_arry = pre_arry.reshape(len(pre_arry),-1,1)
        for i in range(mc_times-1):
            pre_temp = self.model(test_data)
            pre_temp = pre_temp.cpu().data.numpy()
            pre_temp = pre_temp.reshape(len(pre_temp),-1,1)
            pre_arry = np.concatenate((pre_arry,pre_temp),2)
        test_data = test_data.cpu().data.numpy()
        pre_mean = np.mean(pre_arry,2)
        pre_std = np.std(pre_arry,2)
        pre_mean, pre_std = pre_mean.reshape(len(pre_mean),-1), pre_std.reshape(len(pre_std),-1)
        # recover raw data
        if scaler:
        ...
        if is_uncertainty:
            ...
            return pre_mean
    def evaluation_model(self, scaler, test_data, test_label, t=1, confidence=95, mc_times=400):
        """Get predictive intervals and evaluation scores.
        ...
        # self.model = load_model(self.rnn_model,os.path.join(self.model_path, 'checkpoint.pt'))
        pre_arry = self.model(test_data)
        pre_arry, test_label = pre_arry.cpu().data.numpy(), test_label.cpu().data.numpy()
        pre_arry = pre_arry.reshape(len(pre_arry),-1,1)
        for i in range(mc_times-1):
            pre_temp = self.model(test_data)
            pre_temp = pre_temp.cpu().data.numpy()
    ...
    def save_model(self, model_path=Path('./model', 'checkpoint_mc.pt')):
        """Save pytorch model.
    ...
    def load_model(self, path=Path('./model', 'checkpoint_mc.pt')):
        """Load Pytorch model.
