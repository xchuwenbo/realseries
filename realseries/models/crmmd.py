# -*- encoding: utf-8 -*-
"""The crmmd is the implentation of paper
'Calibrated Reliable Regression using Maximum Mean Discrepancy'
https://arxiv.org/abs/2006.10255
"""
import os
...
from ..utils.utility import EarlyStopping, aleatoric_loss, mmd_loss
from .base_rnn import rnn_base
from .base import BaseModel


class CRMMD(BaseModel):
    ...
    def __init__(
        ...
        self.epochs_hnn = epochs_hnn
        self.epochs_mmd = epochs_mmd
    ...
    def fit(self,
        ...
        Args:
            ...
            verbose (bool, optional): Verbose decides what to print. Defaults to
                True.
        Attributes:
            model: a trained model to save to model_path/checkpoint.pt.
        """
        os.makedirs(self.model_path, exist_ok=True)
        early_stopping_hnn = EarlyStopping(patience=patience, delta=delta, verbose=verbose)
        early_stopping_mmd = EarlyStopping(patience=patience, delta=delta, verbose=verbose)
        ...
        # the first step with HNN
        for epoch in range(1, self.epochs_hnn + 1):
            train_batch_losses = []
            valid_batch_losses = []
            ...
            if epoch % 20 == 0:
                print(f'epoch for HNN: {epoch:} -> train_loss: {train_loss:.5f}, valid_loss: {valid_loss:.5f}')

            # early_stopping_hnn call function
            early_stopping_hnn(valid_loss, self.rnn_model)
            if early_stopping_hnn.early_stop:
                print("Early stopping for the first phase")
                break
        
        # the second step with mmd
        for epoch in range(1, self.epochs_mmd + 1):
            train_batch_losses = []
            valid_batch_losses = []
            self.rnn_model.train()
            for data, target in train_loader:
                ...
                loss = mmd_loss_func(target, output)
                self.rnn_model.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.rnn_model.parameters(),self.grad_clip)
                optimizer.step()
            ...
            self.valid_losses.append(valid_loss)
            if epoch % 10 == 0:
                print(f'epoch for MMD: {epoch:} -> train_loss: {train_loss:.5f}, valid_loss: {valid_loss:.5f}')

            # early_stopping_hnn call function
            early_stopping_mmd(valid_loss, self.rnn_model)
            if early_stopping_mmd.early_stop:
                print("Early stopping for the second step")
                break
            if epoch % 200 == 0:
                self.save_model(Path(self.model_path, f'checkpoint{epoch}.pt'))
        self.save_model(Path(self.model_path, 'checkpoint_crmmd.pt'))
    ...
    def save_model(self, model_path=Path('./model', 'checkpoint_crmmd.pt')):
        """Save pytorch model.
    ...
    def load_model(self, path=Path('./model', 'checkpoint_crmmd.pt')):
        """Load Pytorch model.
